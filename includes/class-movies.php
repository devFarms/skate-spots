...
class Skate_Movies {
    ...
    private static $table_name = 'skate_movies';
    ...
    public static function get_all($status = null, $limit = 100, $offset = 0) {
        ...
        if ($status !== null) {
            return $wpdb->get_results($wpdb->prepare(
                "SELECT * FROM $table WHERE status = %d ORDER BY release_date DESC LIMIT %d OFFSET %d",
                $status,
                $limit,
                $offset
            ));
            ...
            return $wpdb->get_results($wpdb->prepare(
                "SELECT * FROM $table ORDER BY release_date DESC LIMIT %d OFFSET %d",
                $limit,
                $offset
            ));
        }
